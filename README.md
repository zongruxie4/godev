...
**LLM-Friendly Full-Stack Go Framework** – Build complete web applications using **only Go + WebAssembly**, with minimal JavaScript. GoLite is a **TUI-based development environment** that acts as an intelligent intermediary between you, your LLM assistant, and your application.
...
![GoLite TUI Preview](docs/tui.JPG)

---

## 🎯 **Why GoLite?**

**GoLite is a TypeScript alternative for full-stack development—but with Go everywhere.**
...
GoLite includes a **fully functional Model Context Protocol server** that allows AI assistants (GitHub Copilot, Claude, etc.) to:
...
The GoLite TUI is your **development control center**:
...
- **No Bundlers**: GoLite handles asset optimization internally
...
> **Future Vision**: GoLite will provide a standalone installer that bundles Go, Git, GitHub CLI, VS Code, and everything needed for development—no manual setup required.
...
# Start GoLite (initializes project structure and TUI)
golite
```

**That's it.** GoLite will:
...
**⚠️ MANDATORY STRUCTURE** - GoLite enforces this convention to eliminate configuration files:
...
GoLite provides the basic WebAssembly bootstrap script automatically. You should **only write vanilla JavaScript when**:
...
GoLite is the **starting point** for a Go-first web ecosystem. Community contributions are encouraged, including:

- **[tinystring](https://github.com/cdvelop/tinystring)** - 80% lighter alternative to `fmt`, `strconv`, `strings`, `bytes.Buffer`, and `errors`
...
GoLite aims to become a **complete Go web development platform**:
...
**That's it!** Modify either file, and GoLite automatically:
...
GoLite is designed for **human + AI collaboration**:

### **Traditional Workflow** (Without GoLite)
```
...
### **GoLite Workflow** (With MCP Integration)
```
Developer: "Add a user login form"
LLM: [Reads project via MCP] 
     ...
     [GoLite auto-compiles and reloads]
...
GoLite welcomes contributions! We're building a **Go-first web ecosystem** and need:
...
GoLite builds on amazing open-source projects:
...
- **Issues**: [GitHub Issues](https://github.com/cdvelop/golite/issues)
- **Discussions**: [GitHub Discussions](https://github.com/cdvelop/golite/discussions)
