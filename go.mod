module github.com/cdvelop/godev

go 1.20

require (
	github.com/cdvelop/gotools v0.0.25
	github.com/chromedp/chromedp v0.9.1
)

require (
	github.com/cdvelop/gomod v0.0.6 // indirect
	github.com/cdvelop/model v0.0.36 // indirect
	github.com/lxn/win v0.0.0-20210218163916-a377121e959e // indirect
	github.com/tdewolff/minify v2.3.6+incompatible // indirect
	github.com/tdewolff/parse v2.3.4+incompatible // indirect
)

require (
	github.com/cdvelop/compiler v0.0.9
	github.com/cdvelop/dev_browser v0.0.2
	github.com/cdvelop/watch_files v0.0.5
	github.com/chromedp/cdproto v0.0.0-20230625224106-7fafe342e117 // indirect
	github.com/chromedp/sysutil v1.0.0 // indirect
	github.com/fsnotify/fsnotify v1.6.0 // indirect
	github.com/fstanis/screenresolution v0.0.0-20190527020317-869904d15333 // indirect
	github.com/gobwas/httphead v0.1.0 // indirect
	github.com/gobwas/pool v0.2.1 // indirect
	github.com/gobwas/ws v1.2.1 // indirect
	github.com/josharian/intern v1.0.0 // indirect
	github.com/mailru/easyjson v0.7.7 // indirect
	golang.org/x/sys v0.10.0 // indirect
)

replace github.com/cdvelop/model => ../model

replace github.com/cdvelop/gotools => ../gotools

replace github.com/cdvelop/input => ../input

replace github.com/cdvelop/platform => ../platform

replace github.com/cdvelop/dev_browser => ../dev_browser

replace github.com/cdvelop/watch_files => ../watch_files

replace github.com/cdvelop/compiler => ../compiler

replace github.com/cdvelop/js => ../js
